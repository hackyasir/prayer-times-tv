...
function buildToranj(cx, cy, r) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const lx  = cx + br * Math.cos(a1), ly = cy + br * Math.sin(a1);
    const rx  = cx + br * Math.cos(a2), ry = cy + br * Math.sin(a2);
  ...
  // centre circle
  const cr = r * 0.08;
  paths.push(`M ${c(cx + cr, cy)} A ${cr.toFixed(2)} ${cr.toFixed(2)} 0 1 1 ${c(cx + cr - .01, cy)} Z`);
...
function buildIslimi(cx, cy, fieldR, arms) {
  ...
  for (let i = 0; i < arms; i++) {
    ...
    const cp1x = lerp(p0x, p1x, .5) - (p1y - p0y) * .35;
    const cp1y = lerp(p0y, p1y, .5) + (p1x - p0x) * .35;
    const cp2x = lerp(p2x, p3x, .5) + (p3y - p2y) * .35;
    const cp2y = lerp(p2y, p3y, .5) - (p3x - p2x) * .35;
    const mcp1x = lerp(p1x, p2x, .3) - (p2y - p1y) * .4;
    const mcp1y = lerp(p1y, p2y, .3) + (p2x - p1x) * .4;
    const mcp2x = lerp(p1x, p2x, .7) + (p2y - p1y) * .4;
    const mcp2y = lerp(p1y, p2y, .7) - (p2x - p1x) * .4;
    ...
    const bcx = lerp(p2x, bx, .5) - (by - p2y) * .4;
    const bcy = lerp(p2y, by, .5) + (bx - p2x) * .4;
...
function buildPalmette(cx, cy, r, a) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const cpx = cx + r * 0.62 * Math.cos(pa + (t - .5) * .28);
    const cpy = cy + r * 0.62 * Math.sin(pa + (t - .5) * .28);
    paths.push(`M ${c(bx, by)} Q ${c(cpx, cpy)} ${c(tx, ty)}`);
  }

  // stamen circle
  const sr = r * 0.11;
  const sx = cx + r * 0.18 * Math.cos(a), sy = cy + r * 0.18 * Math.sin(a);
  paths.push(`M ${c(sx + sr, sy)} A ${sr.toFixed(2)} ${sr.toFixed(2)} 0 1 1 ${c(sx + sr - .01, sy)} Z`);
...
function buildBoteh(cx, cy, r, angle) {
  ...
  const lcp = [lerp(lx, crx, .6) - (ty - ly) * .28, lerp(ly, cry, .6) + (tx - lx) * .28];
  const rcp = [lerp(rx, crx, .6) + (ty - ry) * .28, lerp(ry, cry, .6) - (tx - rx) * .28];
  ...
  return [
    ...
    `M ${c(cx, cy)} Q ${c(lerp(cx, tx, .38), lerp(cy, ty, .38))} ${c(lerp(cx, tx, .68), lerp(cy, ty, .68))}`,
  ];
}

function buildBotehField(W, H, r) {
  const all = [], step = r * 2.35;
...
function buildDecagram(cx, cy, outerR, innerR) {
  ...
  const full  = [];
  for (let i = 0; i < 10; i++) { full.push(outer[i]); full.push(inner[i]); }
  const poly = (pts) => 'M ' + pts.map(([x, y]) => c(x, y)).join(' L ') + ' Z';
...
const LAYERS = {
  ...
  sunrise: { lattice: 0.20, toranj: 0.5, scrolls: 1.0, boteh: 0.3,  deka: 0.2,  border: 0.4 },
  dhuhr:   { lattice: 0.55, toranj: 0.6, scrolls: 0.4, boteh: 0.3,  deka: 1.0,  border: 0.8 },
  asr:     { lattice: 0.30, toranj: 0.5, scrolls: 1.0, boteh: 0.4,  deka: 0.5,  border: 0.6 },
  maghrib: { lattice: 0.45, toranj: 1.0, scrolls: 0.8, boteh: 0.6,  deka: 0.6,  border: 1.0 },
  isha:    { lattice: 0.60, toranj: 0.7, scrolls: 0.3, boteh: 0.4,  deka: 1.0,  border: 0.7 },
};
...
const THEME_PALETTES = {
  ...
  'Emerald Night': { hi: '#7eeaaa', mid: '#2ecc71', lo: '#083820', tint: 0.50 },
  'Royal Blue':    { hi: '#a0c8f8', mid: '#4A90E2', lo: '#081840', tint: 0.50 },
...
function blendPal(prayerPal, themePal) {
  ...
  const mix = (hex1, hex2, t) => {
    ...
    const r = Math.round(r1*(1-t)+r2*t).toString(16).padStart(2,'0');
    const g = Math.round(g1*(1-t)+g2*t).toString(16).padStart(2,'0');
    const b = Math.round(b1*(1-t)+b2*t).toString(16).padStart(2,'0');
    return '#' + r + g + b;
  };
  return {
...
export default function IslamicGeometryEngine({
  ...
  const masterOp = sunElevation > 0
    ? lerp(0.18, 0.26, clamp(sunElevation / 90, 0, 1))
    : lerp(0.22, 0.30, clamp(-sunElevation / 18, 0, 1));

  const sw    = lerp(1.6, 0.9, clamp(sunElevation / 90, 0, 1));
  const scale = minutesToNext <= 30
    ? lerp(1.0, 1.18, clamp(1 - minutesToNext / 30, 0, 1)) : 1.0;
  ...
  const {
    latticePaths = [], toranjPaths = [], scrolls = [], palms = [],
    botehPaths = [], dekaPaths = [], borderPaths = [],
  } = data;

  return (
    <svg ref={svgRef} aria-hidden="true"
      style={{ position:'absolute', inset:0, width:'100%', height:'100%',
               pointerEvents:'none', zIndex:0, overflow:'hidden', transition:'opacity 1s ease' }}
      ...
      <g opacity={masterOp}>
        ...
        {lw.lattice > 0 && (
          <g fill="none" stroke={`url(#${g2Id})`} strokeWidth={sw * 0.45}
            opacity={lw.lattice * 0.55} strokeLinejoin="round">
            {latticePaths.map((d, i) => <path key={`lt${i}`} d={d} />)}
          </g>
        )}
        ...
        {lw.boteh > 0 && (
          <g fill="none" stroke={`url(#${g2Id})`} strokeWidth={sw * 0.8}  opacity={lw.boteh * 0.75}>
            {botehPaths.map((d, i) => <path key={`b${i}`} d={d} />)}
          </g>
        )}

        {/* decagram geometric stars */}
        {lw.deka > 0 && (
          <g fill="none" stroke={`url(#${gId})`} strokeWidth={sw * 0.9}  opacity={lw.deka * 0.88}
            filter={isNight ? `url(#${glId})` : undefined}>
            {dekaPaths.map((d, i) => <path key={`d${i}`} d={d} />)}
          </g>
        )}

        {/* islimi vine scrolls */}
        {lw.scrolls > 0 && (
          <g fill="none" stroke={`url(#${g2Id})`} strokeWidth={sw * 1.1}
            strokeLinecap="round" opacity={lw.scrolls * 0.90}>
            {scrolls.map((d, i) => <path key={`s${i}`} d={d} />)}
          </g>
        )}

        {/* Shah Abbasi palmettes at scroll tips */}
        {lw.scrolls > 0 && (
          <g fill="none" stroke={`url(#${g2Id})`} strokeWidth={sw * 0.8}
            strokeLinecap="round" opacity={lw.scrolls * 0.78}>
            {palms.map((d, i) => <path key={`p${i}`} d={d} />)}
          </g>
        )}
        ...
        {lw.border > 0 && (
          <g fill="none" stroke={`url(#${gId})`} strokeWidth={sw * 1.0}  opacity={lw.border * 0.72}>
            {borderPaths.map((d, i) => <path key={`br${i}`} d={d} />)}
          </g>
        )}

        {/* central Toranj medallion — top layer */}
        {lw.toranj > 0 && (
          <g stroke={`url(#${gId})`} strokeWidth={sw} strokeLinejoin="round"
            fill={`url(#${gId})`} fillOpacity={0.16}  opacity={lw.toranj * 0.95}
            filter={isNight ? `url(#${glId})` : undefined}>
            {toranjPaths.map((d, i) => <path key={`t${i}`} d={d} />)}
          </g>
        )}
        ...
        {lunarPhase > 0.06 && (() => {
          ...
          const lx = W * 0.93, ly = H * 0.06;
          return (
            <g opacity={lunarPhase * 0.07}>
              <circle cx={lx} cy={ly} r={cr.toFixed(1)} fill={accent} />
              {lunarPhase < 0.88 && (
                <circle cx={lx - Math.min(W, H) * 0.028 * (1 - lunarPhase)} cy={ly}
                  r={(cr * 0.94).toFixed(1)} fill="var(--t-bg,#000)" />
              )}
            </g>
          );
        })()}
