...
// At build time, Workbox replaces this with generated asset entries.
const INJECTED_ASSETS = (self.__WB_MANIFEST || []).map((entry) => entry.url);

// ── Install: pre-cache shell ──────────────────────────────────────────────────
self.addEventListener('install', (event) => {
  const assetsToCache = [...new Set([...PRECACHE_ASSETS, ...INJECTED_ASSETS])];
  event.waitUntil(
